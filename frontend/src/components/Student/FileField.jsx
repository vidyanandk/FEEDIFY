...
function FileField({ fieldModel, onCompleted }) {
  ...
  const handleFile = async (e) => {
    ...
    if (!file) return;
    if (fieldModel.accepted.indexOf(ext) === -1)
      return setErr(
        "Choose file with thoe following extensions: " +
          fieldModel.accepted.join(", ")
      );
    //chechk if file size is biggern than  2MB
    if (file.size > 2 * 1024 * 1024)
      return setErr("File size should be less than than 2MB");
    ...
    <div className="input">
      ...
      <div className="file-field">
        <input
          ...
          onChange={handleFile}
        />
        <label className="btn" htmlFor={fieldModel.title.replace(" ", "")}>
          choose file
        </label>
